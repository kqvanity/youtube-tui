...
pub trait Filter {
    fn matches(&self, _black_list: BlockList) -> bool {
        false
    }
}
...
impl Filter for Item {
    fn matches(&self, black_list: BlockList) -> bool {
        let blocked_channels =
            crate::global::structs::DatabaseManager::get_all_blocked_channels().unwrap_or_default();
        let blocked_playlists =
            crate::global::structs::DatabaseManager::get_all_blocked_playlists()
                .unwrap_or_default();
        ...
        blocked_channels.contains(&result.channel_id)
            ...
            || black_list
                .keywords
                .contains(&result.channel_title.to_lowercase())
            || black_list
                .keywords
                .iter()
                .any(|x| result.title.to_lowercase().contains(x))
    }
}
